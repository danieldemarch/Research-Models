...
x = concatenate([outm, outd])
out = Dense(1, activation = 'sigmoid')(x)

model = Model(inputs=[inm, input_img], outputs=[out])
...
start = time.time()
history = model.fit(x = [mXt, dXt], y = myt,
                                  epochs=epochs,
                                  validation_data=([mX_val, dX_val],my_val_s),
                                  callbacks=[reduce_lr])
stop = time.time()
time_elapsed = stop - start

print("########################")
my_predict = model.predict([mX_test, dX_test])
